      ...
      {items.map((item, index) => {
        ...
        return (
          <button
            ...
            className={cx(
              'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-pill text-sm font-bold transition-colors',
              size === 44 ? 'min-h-11 px-3.5' : 'min-h-10 px-[18px]',
              fill && 'flex-1',
              active ? 'bg-surface text-ink shadow-tab' : 'text-ink2 hover:text-ink',
            )}
          >
            {item.label}
          </button>
